#include QMK_KEYBOARD_H
#include "andy.h"
#include "tapdance.h"
#include "encoder.c"

// qmk flash -kb sofle -km andy
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BASE] = LAYOUT(
    KC_BTN1, KC_BTN3, KC_BTN2, TD(C35), KC_WH_U, KC_WH_D,                   TD(VI6), TD(SL7), TD(SL8), CMD_9,  CMD_0,   KC_PGUP,
    TD(C12), KC_Q,    KC_W,    KC_F,    KC_P,    KC_V,                      KC_J,    KC_L,    KC_U,    KC_Y,   KC_SCLN, KC_PGDN,
    ESC_SC,  KC_A,    KC_R,    KC_S,    KC_T,    KC_G,                      KC_M,    KC_N,    KC_E,    KC_I,   KC_O,    QUOT_SC,
    KC_LCMD, Z_SFT,   KC_X,    KC_C,    KC_D,    KC_B,    TD(APP), TD(TAB), KC_K,    KC_H,    KC_COMM, KC_DOT, SLH_SFT, KC_RCMD,
                      PST_FND, CPY_CUT, TAB_NUM, SPC_CTL, BSP_ALT, TD(MUX), ENT_NAV, NUM_SYM, TD(WIN), SLAYER
  ),

  [_SHORTCUT] = LAYOUT(
    XXXXXXX, FFGPREV, FFGNEXT, XXXXXXX, XXXXXXX, XXXXXXX,                  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_HOME,
    XXXXXXX, XXXXXXX, XXXXXXX, FIREFOX, PDF,     VLC,                      XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_END,
    KC_CAPS, XXXXXXX, XXXXXXX, FINDER,  XXXXXXX, CHROME,                   XXXXXXX, NOTES,   XXXXXXX, ITERM,   OUTLOOK, KC_PSCR,
    XXXXXXX, _______, EXCEL,   CHIME,   DBEAVER, PAINT,  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, _______, XXXXXXX,
                      KC_VOLD, KC_VOLU, TAB_UP,  TAB_DN, KC_DEL,  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
  ),

  [_NUMSYM] = LAYOUT(
    KC_F11,  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,                     KC_F6,   KC_F7, KC_F8,   KC_F9,   KC_F10,  KC_F12,
    XXXXXXX, KC_LBRC, KC_RBRC, KC_HASH, KC_CIRC, KC_PERC,                   KC_BSLS, KC_7,  KC_8,    KC_9,    KC_PLUS, KC_TILD,
    XXXXXXX, KC_LPRN, KC_RPRN, KC_EXLM, KC_DLR,  KC_ASTR,                   KC_MINS, KC_4,  KC_5,    KC_6,    KC_EQL,  KC_GRV,
    _______, KC_LCBR, KC_RCBR, KC_PIPE, KC_AMPR, KC_AT,   XXXXXXX, XXXXXXX, KC_UNDS, KC_1,  KC_2,    KC_3,    KC_DOT,  _______,
                      XXXXXXX, XXXXXXX, XXXXXXX, _______, KC_DEL,  KC_TILD, _______, KC_0,  XXXXXXX, XXXXXXX
  ),

  [_NAVIGATE] = LAYOUT(
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,                   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, XXXXXXX, LNSTART, KC_UP,   LNEND,   XXXXXXX,                   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, XXXXXXX, KC_LEFT, KC_DOWN, KC_RGHT, TD(LNK),                   XXXXXXX, KC_LSFT, KC_ESC,  XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, XXXXXXX, WNDPREV, XXXXXXX, WNDNEXT, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
                      XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
  ),

  [_SLAYER] = LAYOUT(
    RESET,   SL_1,    SL_2,    SL_3,    SL_4,    SL_5,                      XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, SL_PU,
    XXXXXXX, XXXXXXX, SL_W,    XXXXXXX, SL_P,    XXXXXXX,                   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, SL_PD,
    XXXXXXX, SL_A,    XXXXXXX, XXXXXXX, XXXXXXX, SL_G,                      SL_M,    SL_N,    SL_E,    SL_I,    SL_O,    XXXXXXX,
    XXXXXXX, SL_Z,    XXXXXXX, SL_C,    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, SL_K,    SL_H,    SL_CO,   SL_PE,   SL_SL,   XXXXXXX,
                      XXXXXXX, XXXXXXX, XXXXXXX, KC_LSFT, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
  ),
};
